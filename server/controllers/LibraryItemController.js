...
class LibraryItemController {
  ...
  async findOne(req, res) {
    ...
    if (req.query.expanded == 1) {
      ...
      if (includeEntities.includes("progress")) {
        ...
        item.userMediaProgress = req.user.getOldMediaProgress(
          item.id,
          episodeId,
        );
      }
      ...
      if (
        item.mediaType === "book" &&
        req.user.isAdminOrUp &&
        includeEntities.includes("share")
      ) {
      ...
      if (
        ...
        includeEntities.includes("downloads")
      ) {
        const downloadsInQueue = this.podcastManager.getEpisodeDownloadsInQueue(
          req.libraryItem.id,
        );
        item.episodeDownloadsQueued = downloadsInQueue.map((d) =>
          d.toJSONForClient(),
        );
        if (
          this.podcastManager.currentDownload?.libraryItemId ===
          req.libraryItem.id
        ) {
          item.episodesDownloading = [
            this.podcastManager.currentDownload.toJSONForClient(),
          ];
        }
      }
  ...
  async delete(req, res) {
    ...
    if (req.libraryItem.isPodcast) {
      mediaItemIds.push(
        ...req.libraryItem.media.podcastEpisodes.map((ep) => ep.id),
      );
    } else {
    ...
    await this.handleDeleteLibraryItem(
      req.libraryItem.id,
      mediaItemIds,
      req.libraryItem.libraryId,
    );
    if (hardDelete) {
      Logger.info(
        `[LibraryItemController] Deleting library item from file system at "${libraryItemPath}"`,
      );
      await fs.remove(libraryItemPath).catch((error) => {
        Logger.error(
          `[LibraryItemController] Failed to delete library item from file system at "${libraryItemPath}"`,
          error,
        );
      });
    }
  ...
  async download(req, res) {
    if (!req.user.canDownload) {
      Logger.warn(
        `User "${req.user.username}" attempted to download without permission`,
      );
    ...
    try {
      ...
      if (req.libraryItem.isFile) {
        ...
        const audioMimeType = getAudioMimeTypeFromExtname(
          Path.extname(libraryItemPath),
        );
        if (audioMimeType) {
          res.setHeader("Content-Type", audioMimeType);
        }
        await new Promise((resolve, reject) =>
          res.download(libraryItemPath, req.libraryItem.relPath, (error) =>
      ...
      Logger.info(
        `[LibraryItemController] Downloaded item "${itemTitle}" at "${libraryItemPath}"`,
      );
    } catch (error) {
      Logger.error(
        `[LibraryItemController] Download failed for item "${itemTitle}" at "${libraryItemPath}"`,
        error,
      );
  ...
  async updateMedia(req, res) {
    ...
    if (mediaPayload.url) {
      await LibraryItemController.prototype.uploadCover.bind(this)(
        req,
        res,
        false,
      );
    ...
    if (req.libraryItem.isPodcast) {
      if (
        mediaPayload.autoDownloadEpisodes !== undefined &&
        req.libraryItem.media.autoDownloadEpisodes !==
          mediaPayload.autoDownloadEpisodes
      ) {
        isPodcastAutoDownloadUpdated = true;
      } else if (
        mediaPayload.autoDownloadSchedule !== undefined &&
        req.libraryItem.media.autoDownloadSchedule !==
          mediaPayload.autoDownloadSchedule
      ) {
        isPodcastAutoDownloadUpdated = true;
      }
      ...
      if (
        mediaPayload.autoDownloadSchedule &&
        !cron.validate(mediaPayload.autoDownloadSchedule)
      ) {
        Logger.error(
          `[LibraryItemController] Invalid auto download schedule cron expression "${mediaPayload.autoDownloadSchedule}" for library item "${req.libraryItem.media.title}"`,
        );
        return res
          .status(400)
          .send("Invalid auto download schedule cron expression");
      }
    }

    let hasUpdates =
      (await req.libraryItem.media.updateFromRequest(mediaPayload)) ||
      mediaPayload.url;

    if (
      req.libraryItem.isBook &&
      Array.isArray(mediaPayload.metadata?.series)
    ) {
      const seriesUpdateData =
        await req.libraryItem.media.updateSeriesFromRequest(
          mediaPayload.metadata.series,
          req.libraryItem.libraryId,
        );
      if (seriesUpdateData?.seriesRemoved.length) {
        // Check remove empty series
        Logger.debug(
          `[LibraryItemController] Series were removed from book. Check if series are now empty.`,
        );
        await this.checkRemoveEmptySeries(
          seriesUpdateData.seriesRemoved.map((se) => se.id),
        );
      }
      if (seriesUpdateData?.seriesAdded.length) {
        // Add series to filter data
        seriesUpdateData.seriesAdded.forEach((se) => {
          Database.addSeriesToFilterData(
            req.libraryItem.libraryId,
            se.name,
            se.id,
          );
    ...
    if (
      req.libraryItem.isBook &&
      Array.isArray(mediaPayload.metadata?.authors)
    ) {
      const authorNames = mediaPayload.metadata.authors
        .map((au) => (typeof au.name === "string" ? au.name.trim() : null))
        .filter((au) => au);
      const authorUpdateData =
        await req.libraryItem.media.updateAuthorsFromRequest(
          authorNames,
          req.libraryItem.libraryId,
        );
      if (authorUpdateData?.authorsRemoved.length) {
        ...
        Logger.debug(
          `[LibraryItemController] Authors were removed from book. Check if authors are now empty.`,
        );
        await this.checkRemoveAuthorsWithNoBooks(
          authorUpdateData.authorsRemoved.map((au) => au.id),
        );
        hasUpdates = true;
      }
      if (authorUpdateData?.authorsAdded.length) {
        ...
        authorUpdateData.authorsAdded.forEach((au) => {
          Database.addAuthorToFilterData(
            req.libraryItem.libraryId,
            au.name,
            au.id,
          );
    ...
    if (hasUpdates) {
      ...
      Logger.debug(
        `[LibraryItemController] Updated library item media ${req.libraryItem.media.title}`,
      );
  ...
  async uploadCover(req, res, updateAndReturnJson = true) {
    if (!req.user.canUpload) {
      Logger.warn(
        `User "${req.user.username}" attempted to upload a cover without permission`,
      );
      return res.sendStatus(403);
    }
    ...
    if (req.body?.url) {
      Logger.debug(
        `[LibraryItemController] Requesting download cover from url "${req.body.url}"`,
      );
      result = await CoverManager.downloadCoverFromUrlNew(
        req.body.url,
        req.libraryItem.id,
        req.libraryItem.isFile ? null : req.libraryItem.path,
      );
  ...
  async updateCover(req, res) {
    ...
    const validationResult = await CoverManager.validateCoverPath(
      req.body.cover,
      req.libraryItem,
    );
  ...
  async getCover(req, res) {
    ...
    if (raw) {
      const coverPath =
        await Database.libraryItemModel.getCoverPath(libraryItemId);
      ...
      if (global.XAccel) {
        ...
        return res
          .status(204)
          .header({ "X-Accel-Redirect": encodedURI })
          .send();
      }
  ...
  startPlaybackSession(req, res) {
    if (!req.libraryItem.hasAudioTracks) {
      Logger.error(
        `[LibraryItemController] startPlaybackSession cannot playback ${req.libraryItem.id}`,
      );
      return res.sendStatus(404);
    }
  ...
  startEpisodePlaybackSession(req, res) {
    if (!req.libraryItem.isPodcast) {
      Logger.error(
        `[LibraryItemController] startEpisodePlaybackSession invalid media type ${req.libraryItem.id}`,
      );
      return res.sendStatus(400);
    }
    ...
    if (
      !req.libraryItem.media.podcastEpisodes.some((ep) => ep.id === episodeId)
    ) {
      Logger.error(
        `[LibraryItemController] startPlaybackSession episode ${episodeId} not found for item ${req.libraryItem.id}`,
      );
      return res.sendStatus(404);
    }
  ...
  async updateTracks(req, res) {
    ...
    if (!req.libraryItem.isBook) {
      Logger.error(
        `[LibraryItemController] updateTracks invalid media type ${req.libraryItem.id}`,
      );
      return res.sendStatus(400);
    }
    if (!Array.isArray(orderedFileData) || !orderedFileData.length) {
      Logger.error(
        `[LibraryItemController] updateTracks invalid orderedFileData ${req.libraryItem.id}`,
      );
      return res.sendStatus(400);
    }
    // Ensure that each orderedFileData has a valid ino and is in the book audioFiles
    if (
      orderedFileData.some(
        (fileData) =>
          !fileData?.ino ||
          !req.libraryItem.media.audioFiles.some(
            (af) => af.ino === fileData.ino,
          ),
      )
    ) {
      Logger.error(
    ...
    const updatedAudioFiles = orderedFileData.map((fileData) => {
      const audioFile = req.libraryItem.media.audioFiles.find(
        (af) => af.ino === fileData.ino,
      );
  ...
  async match(req, res) {
    ...
    const matchResult = await Scanner.quickMatchLibraryItem(
      this,
      req.libraryItem,
      options,
    );
  ...
  async batchDelete(req, res) {
    if (!req.user.canDelete) {
      Logger.warn(
        `[LibraryItemController] User "${req.user.username}" attempted to delete without permission`,
      );
      return res.sendStatus(403);
    }
    ...
    for (const libraryItem of itemsToDelete) {
      ...
      Logger.info(
        `[LibraryItemController] (${hardDelete ? "Hard" : "Soft"}) deleting Library Item "${libraryItem.media.title}" with id "${libraryItem.id}"`,
      );
      const mediaItemIds = [];
      const seriesIds = [];
      const authorIds = [];
      if (libraryItem.isPodcast) {
        mediaItemIds.push(
          ...libraryItem.media.podcastEpisodes.map((ep) => ep.id),
        );
      } else {
  ...
  async batchUpdate(req, res) {
    if (!req.user.canUpdate) {
      Logger.warn(
        `[LibraryItemController] User "${req.user.username}" attempted to batch update without permission`,
      );
      return res.sendStatus(403);
    }
    ...
    if (!Array.isArray(updatePayloads) || !updatePayloads.length) {
      Logger.error(
        `[LibraryItemController] Batch update failed. Invalid payload`,
      );
      return res.sendStatus(400);
    }
    ...
    const libraryItemIds = [
      ...new Set(updatePayloads.map((up) => up?.id).filter((id) => id)),
    ];
    if (
      !libraryItemIds.length ||
      libraryItemIds.length !== updatePayloads.length
    ) {
      Logger.error(
        `[LibraryItemController] Batch update failed. Each update payload must have a unique library item id`,
      );
      return res.sendStatus(400);
    }
    ...
    if (updatePayloads.length !== libraryItems.length) {
      Logger.error(
        `[LibraryItemController] Batch update failed. Not all library items found`,
      );
      return res.sendStatus(404);
    }
    ...
    for (const updatePayload of updatePayloads) {
      ...
      if (
        libraryItem.isPodcast &&
        mediaPayload.autoDownloadSchedule &&
        !cron.validate(mediaPayload.autoDownloadSchedule)
      ) {
        Logger.warn(
          `[LibraryItemController] Invalid auto download schedule cron expression "${mediaPayload.autoDownloadSchedule}" for library item "${libraryItem.media.title}" - skipping update`,
        );
        continue;
      }
      ...
      if (libraryItem.isBook && Array.isArray(mediaPayload.metadata?.series)) {
        const seriesUpdateData =
          await libraryItem.media.updateSeriesFromRequest(
            mediaPayload.metadata.series,
            libraryItem.libraryId,
          );
        if (seriesUpdateData?.seriesRemoved.length) {
          seriesIdsRemoved.push(
            ...seriesUpdateData.seriesRemoved.map((se) => se.id),
          );
        }
        if (seriesUpdateData?.seriesAdded.length) {
      ...
      if (libraryItem.isBook && Array.isArray(mediaPayload.metadata?.authors)) {
        const authorNames = mediaPayload.metadata.authors
          ...
          .filter((au) => au);
        const authorUpdateData =
          await libraryItem.media.updateAuthorsFromRequest(
            authorNames,
            libraryItem.libraryId,
          );
        if (authorUpdateData?.authorsRemoved.length) {
          authorIdsRemoved.push(
            ...authorUpdateData.authorsRemoved.map((au) => au.id),
          );
          hasUpdates = true;
        }
      ...
      if (hasUpdates) {
        ...
        Logger.debug(
          `[LibraryItemController] Updated library item media "${libraryItem.media.title}"`,
        );
  ...
  async batchQuickMatch(req, res) {
    if (!req.user.isAdminOrUp) {
      Logger.warn(
        `Non-admin user "${req.user.username}" other than admin attempted to batch quick match library items`,
      );
      return res.sendStatus(403);
    }
    ...
    const options = {};
    if (
      reqBodyOptions.provider &&
      typeof reqBodyOptions.provider === "string"
    ) {
  ...
  async batchScan(req, res) {
    if (!req.user.isAdminOrUp) {
      Logger.warn(
        `Non-admin user "${req.user.username}" other than admin attempted to batch scan library items`,
      );
      return res.sendStatus(403);
    }
    ...
    for (const libraryItem of libraryItems) {
      if (libraryItem.isFile) {
        Logger.warn(
          `[LibraryItemController] Re-scanning file library items not yet supported`,
        );
      } else {
  ...
  async scan(req, res) {
    if (!req.user.isAdminOrUp) {
      Logger.error(
        `[LibraryItemController] Non-admin user "${req.user.username}" attempted to scan library item`,
      );
      return res.sendStatus(403);
    }
  ...
  getMetadataObject(req, res) {
    if (!req.user.isAdminOrUp) {
      Logger.error(
        `[LibraryItemController] Non-admin user "${req.user.username}" attempted to get metadata object`,
      );
      return res.sendStatus(403);
    }

    if (
      req.libraryItem.isMissing ||
      !req.libraryItem.isBook ||
      !req.libraryItem.media.includedAudioFiles.length
    ) {
      Logger.error(
        `[LibraryItemController] getMetadataObject: Invalid library item "${req.libraryItem.media.title}"`,
      );
      return res.sendStatus(400);
    }

    res.json(
      this.audioMetadataManager.getMetadataObjectForApi(req.libraryItem),
    );
  }
  ...
  async updateMediaChapters(req, res) {
    if (!req.user.canUpdate) {
      Logger.error(
        `[LibraryItemController] User "${req.user.username}" attempted to update chapters with invalid permissions`,
      );
      return res.sendStatus(403);
    }
    ...
    if (
      req.libraryItem.isMissing ||
      !req.libraryItem.isBook ||
      !req.libraryItem.media.hasAudioTracks
    ) {
    ...
    if (
      !Array.isArray(req.body.chapters) ||
      req.body.chapters.some(
        (c) =>
          !c.title ||
  ...
  async getFFprobeData(req, res) {
    if (!req.user.isAdminOrUp) {
      Logger.error(
        `[LibraryItemController] Non-admin user "${req.user.username}" attempted to get ffprobe data`,
      );
      return res.sendStatus(403);
    }
    ...
    if (!audioFile) {
      Logger.error(
        `[LibraryItemController] Audio file not found with inode value ${req.params.fileid}`,
      );
      return res.sendStatus(404);
    }

    const ffprobeData = await AudioFileScanner.probeAudioFile(
  ...
  async getLibraryFile(req, res) {
    ...
    if (global.XAccel) {
      const encodedURI = encodeUriPath(
        global.XAccel + libraryFile.metadata.path,
      );
      Logger.debug(`Use X-Accel to serve static file ${encodedURI}`);
    ...
    // Express does not set the correct mimetype for m4b files so use our defined mimetypes if available
    const audioMimeType = getAudioMimeTypeFromExtname(
      Path.extname(libraryFile.metadata.path),
    );
    if (audioMimeType) {
      res.setHeader("Content-Type", audioMimeType);
    }
  ...
  async deleteLibraryFile(req, res) {
    ...
    Logger.info(
      `[LibraryItemController] User "${req.user.username}" requested file delete at "${libraryFile.metadata.path}"`,
    );

    await fs.remove(libraryFile.metadata.path).catch((error) => {
      Logger.error(
        `[LibraryItemController] Failed to delete library file at "${libraryFile.metadata.path}"`,
        error,
      );
    });

    req.libraryItem.libraryFiles = req.libraryItem.libraryFiles.filter(
      (lf) => lf.ino !== req.params.fileid,
    );
    ...
    if (req.libraryItem.isBook) {
      if (
        req.libraryItem.media.audioFiles.some(
          (af) => af.ino === req.params.fileid,
        )
      ) {
        req.libraryItem.media.audioFiles =
          req.libraryItem.media.audioFiles.filter(
            (af) => af.ino !== req.params.fileid,
          );
        req.libraryItem.media.changed("audioFiles", true);
      ...
      req.libraryItem.media.podcastEpisodes.some(
        (ep) => ep.audioFile.ino === req.params.fileid,
      )
    ) {
      const episodeToRemove = req.libraryItem.media.podcastEpisodes.find(
        (ep) => ep.audioFile.ino === req.params.fileid,
      );
      // Remove episode from all playlists
      await Database.playlistModel.removeMediaItemsFromPlaylists([
        episodeToRemove.id,
      ]);
      ...
      if (numProgressRemoved > 0) {
        Logger.info(
          `[LibraryItemController] Removed media progress for episode ${episodeToRemove.id}`,
        );
      }
      ...
      req.libraryItem.media.podcastEpisodes =
        req.libraryItem.media.podcastEpisodes.filter(
          (ep) => ep.audioFile.ino !== req.params.fileid,
        );
    }
  ...
  async downloadLibraryFile(req, res) {
    ...
    if (!req.user.canDownload) {
      Logger.error(
        `[LibraryItemController] User "${req.user.username}" without download permission attempted to download file "${libraryFile.metadata.path}"`,
      );
      return res.sendStatus(403);
    }

    Logger.info(
      `[LibraryItemController] User "${req.user.username}" requested download for item "${req.libraryItem.media.title}" file at "${libraryFile.metadata.path}"`,
    );
    ...
    // Express does not set the correct mimetype for m4b files so use our defined mimetypes if available
    let audioMimeType = getAudioMimeTypeFromExtname(
      Path.extname(libraryFile.metadata.path),
    );
    if (audioMimeType) {
    ...
    try {
      await new Promise((resolve, reject) =>
        res.download(
          libraryFile.metadata.path,
          libraryFile.metadata.filename,
          (error) => (error ? reject(error) : resolve()),
        ),
      );
      Logger.info(
        `[LibraryItemController] Downloaded file "${libraryFile.metadata.path}"`,
      );
    } catch (error) {
      Logger.error(
        `[LibraryItemController] Failed to download file "${libraryFile.metadata.path}"`,
        error,
      );
      LibraryItemController.handleDownloadError(error, res);
    }
  ...
  async getEBookFile(req, res) {
    ...
    if (req.params.fileid) {
      ...
      if (!ebookFile?.isEBookFile) {
        Logger.error(
          `[LibraryItemController] Invalid ebook file id "${req.params.fileid}"`,
        );
    ...
    if (!ebookFile) {
      Logger.error(
        `[LibraryItemController] No ebookFile for library item "${req.libraryItem.media.title}"`,
      );
      return res.sendStatus(404);
    }
    ...
    Logger.info(
      `[LibraryItemController] User "${req.user.username}" requested download for item "${req.libraryItem.media.title}" ebook at "${ebookFilePath}"`,
    );
    ...
    try {
      await new Promise((resolve, reject) =>
        res.sendFile(ebookFilePath, (error) =>
          error ? reject(error) : resolve(),
        ),
      );
      Logger.info(
        `[LibraryItemController] Downloaded ebook file "${ebookFilePath}"`,
      );
    } catch (error) {
      Logger.error(
        `[LibraryItemController] Failed to download ebook file "${ebookFilePath}"`,
        error,
      );
      LibraryItemController.handleDownloadError(error, res);
    }
  ...
  async updateEbookFileStatus(req, res) {
    if (!req.libraryItem.isBook) {
      Logger.error(
        `[LibraryItemController] Invalid media type for ebook file status update`,
      );
      return res.sendStatus(400);
    }
    ...
    if (ebookLibraryFile.isSupplementary) {
      Logger.info(
        `[LibraryItemController] Updating ebook file "${ebookLibraryFile.metadata.filename}" to primary`,
      );
      ...
      Logger.info(
        `[LibraryItemController] Updating ebook file "${ebookLibraryFile.metadata.filename}" to supplementary`,
      );
    }
  ...
  async middleware(req, res, next) {
    req.libraryItem = await Database.libraryItemModel.getExpandedById(
      req.params.id,
    );
    ...
    if (req.params.fileid) {
      req.libraryFile = req.libraryItem.getLibraryFileWithIno(
        req.params.fileid,
      );
      if (!req.libraryFile) {
        Logger.error(
          `[LibraryItemController] Library file "${req.params.fileid}" does not exist for library item`,
        );
        return res.sendStatus(404);
      }
    }
    ...
    if (req.path.includes("/play")) {
      ...
      Logger.warn(
        `[LibraryItemController] User "${req.user.username}" attempted to update without permission`,
      );
      return res.sendStatus(403);
    }
